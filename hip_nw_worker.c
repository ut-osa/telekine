    ...
    hipEvent_t stop)
{
    hipError_t ret;
    ret =
        __do_c_hipHccModuleLaunchKernel(f, globalWorkSizeX, globalWorkSizeY, globalWorkSizeZ, localWorkSizeX,
        localWorkSizeY, localWorkSizeZ, sharedMemBytes, stream, kernelParams, extra, extra_size, start, stop);
...
static hipError_t
__wrapper___do_c_hipModuleLaunchKernel(hipFunction_t * f, unsigned int gridDimX, unsigned int gridDimY,
    unsigned int gridDimZ, unsigned int blockDimX, unsigned int blockDimY, unsigned int blockDimZ,
    unsigned int sharedMemBytes, hipStream_t stream, void **kernelParams, size_t extra_size, char *extra)
{
    hipError_t ret;
    ret =
        __do_c_hipModuleLaunchKernel(f, gridDimX, gridDimY, gridDimZ, blockDimX, blockDimY, blockDimZ, sharedMemBytes,
        stream, kernelParams, extra, extra_size);
...
__handle_command_hip(struct command_base *__cmd)
{
    ...
    case CALL_HIP_HIP_MEMCPY2_D_ASYNC:{
        ava_is_in = 1;
        ava_is_out = 0;
        GPtrArray *__ava_alloc_list_hipMemcpy2DAsync = g_ptr_array_new_full(0, free);
        struct hip_hip_memcpy2_d_async_call *__call = (struct hip_hip_memcpy2_d_async_call *)__cmd;
        assert(__call->base.api_id == HIP_API);
        assert(__call->base.command_size == sizeof(struct hip_hip_memcpy2_d_async_call));
#ifdef AVA_RECORD_REPLAY
        ...
        /* Input: size_t dpitch */
        size_t dpitch;
        dpitch = __call->dpitch;
        dpitch = __call->dpitch;

        /* Input: size_t spitch */
        size_t spitch;
        spitch = __call->spitch;
        spitch = __call->spitch;
        ...
        if (((kind == hipMemcpyDeviceToHost) ? (NW_BUFFER) : (NW_OPAQUE)) == (NW_BUFFER)) {
            if (__call->dst != NULL) {
                const size_t __size = ((size_t) (kind == hipMemcpyDeviceToHost) ? (width * height) : (0));
                dst = (void *)calloc(__size, sizeof(void));
                g_ptr_array_add(__ava_alloc_list_hipMemcpy2DAsync, dst);
            } else {
                dst = NULL;
            }
        } else {
        ...
        if (((kind == hipMemcpyHostToDevice) ? (NW_BUFFER) : (NW_OPAQUE)) == (NW_BUFFER)
            ...
            if (((kind == hipMemcpyHostToDevice) ? (NW_BUFFER) : (NW_OPAQUE)) == (NW_BUFFER)) {
                if (__call->src != NULL) {
                    const size_t __size = ((size_t) (kind == hipMemcpyHostToDevice) ? (width * height) : (0));
                    src = (void *)calloc(__size, sizeof(const void));
                    g_ptr_array_add(__ava_alloc_list_hipMemcpy2DAsync, src);
                    if (kind == hipMemcpyHostToDevice) {
                        void *__tmp_src_0;
                        __tmp_src_0 = (((kind == hipMemcpyHostToDevice) ? (NW_BUFFER) : (NW_OPAQUE)) == (NW_BUFFER)
                            && (__call->src) != (NULL)) ? (((const void *)command_channel_get_buffer(__chan, __cmd,
        ...
        struct hip_hip_memcpy2_d_async_ret *__ret =
            (struct hip_hip_memcpy2_d_async_ret *)command_channel_new_command(__chan,
            sizeof(struct hip_hip_memcpy2_d_async_ret), __total_buffer_size);
        __ret->base.api_id = HIP_API;
        __ret->base.command_id = RET_HIP_HIP_MEMCPY2_D_ASYNC;
        __ret->__call_id = __call->__call_id;
        ...
        if (((kind == hipMemcpyDeviceToHost) ? (NW_BUFFER) : (NW_OPAQUE)) == (NW_BUFFER)) {
            ...
            if (((kind == hipMemcpyDeviceToHost) ? (NW_BUFFER) : (NW_OPAQUE)) == (NW_BUFFER)) {
                if (kind == hipMemcpyDeviceToHost
                    && ((kind == hipMemcpyDeviceToHost) ? (NW_BUFFER) : (NW_OPAQUE)) == (NW_BUFFER) && (dst) != (NULL)
                    && ((kind == hipMemcpyDeviceToHost) ? (width * height) : (0)) > (0)) {
                    const size_t __size = (kind == hipMemcpyDeviceToHost) ? (width * height) : (0);
                    void *__tmp_dst_0;
                    __tmp_dst_0 = (void *)calloc(__size, sizeof(void));
                    g_ptr_array_add(__ava_alloc_list_hipMemcpy2DAsync, __tmp_dst_0);
                    const size_t __dst_size_0 = (__size);
                    for (size_t __dst_index_0 = 0; __dst_index_0 < __dst_size_0; __dst_index_0++) {
                        const size_t ava_index = __dst_index_0;
    ...
    case CALL_HIP___DO_C_HIP_HCC_MODULE_LAUNCH_KERNEL:{
        ...
        globalWorkSizeX = __call->globalWorkSizeX;
        globalWorkSizeX = __call->globalWorkSizeX;

        /* Input: uint32_t globalWorkSizeY */
        uint32_t globalWorkSizeY;
        globalWorkSizeY = __call->globalWorkSizeY;
        globalWorkSizeY = __call->globalWorkSizeY;

        /* Input: uint32_t globalWorkSizeZ */
        uint32_t globalWorkSizeZ;
        globalWorkSizeZ = __call->globalWorkSizeZ;
        globalWorkSizeZ = __call->globalWorkSizeZ;

        /* Input: uint32_t localWorkSizeX */
        uint32_t localWorkSizeX;
        localWorkSizeX = __call->localWorkSizeX;
        localWorkSizeX = __call->localWorkSizeX;

        /* Input: uint32_t localWorkSizeY */
        uint32_t localWorkSizeY;
        localWorkSizeY = __call->localWorkSizeY;
        localWorkSizeY = __call->localWorkSizeY;

        /* Input: uint32_t localWorkSizeZ */
        uint32_t localWorkSizeZ;
        localWorkSizeZ = __call->localWorkSizeZ;
        localWorkSizeZ = __call->localWorkSizeZ;
    ...
    case CALL_HIP___DO_C_HIP_MODULE_LAUNCH_KERNEL:{
        ava_is_in = 1;
        ava_is_out = 0;
        GPtrArray *__ava_alloc_list___do_c_hipModuleLaunchKernel = g_ptr_array_new_full(0, free);
        struct hip___do_c_hip_module_launch_kernel_call *__call =
            (struct hip___do_c_hip_module_launch_kernel_call *)__cmd;
        assert(__call->base.api_id == HIP_API);
        assert(__call->base.command_size == sizeof(struct hip___do_c_hip_module_launch_kernel_call));
#ifdef AVA_RECORD_REPLAY
        ...
        hipError_t ret;
        ret =
            __wrapper___do_c_hipModuleLaunchKernel(f, gridDimX, gridDimY, gridDimZ, blockDimX, blockDimY, blockDimZ,
            sharedMemBytes, stream, kernelParams, extra_size, extra);

        ava_is_in = 0;
        ava_is_out = 1;
        size_t __total_buffer_size = 0; {
        }
        struct hip___do_c_hip_module_launch_kernel_ret *__ret =
            (struct hip___do_c_hip_module_launch_kernel_ret *)command_channel_new_command(__chan,
            sizeof(struct hip___do_c_hip_module_launch_kernel_ret), __total_buffer_size);
        __ret->base.api_id = HIP_API;
        __ret->base.command_id = RET_HIP___DO_C_HIP_MODULE_LAUNCH_KERNEL;
        __ret->__call_id = __call->__call_id;
