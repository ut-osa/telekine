 ...
 * already been called.
 */
void register_command_handler(int api_id, void (*handle_command)(struct command_base* cmd));

/**
 * Block until a command with the specified API is executed.
 */
void handle_commands_until_api(int api_id);

void _internal_handle_commands_until_api(int api_id);
extern pthread_mutex_t nw_handler_lock;
...
#define handle_commands_until(api_id, predicate)                        \
    ...
    while(!(predicate)) _internal_handle_commands_until_api(api_id);    \
